...
describe('Lettering and gentle edges',()=>{
 ...
 it('protects export against unsupported lettering',async()=>{const p=preset('stars-air');p.lettering.text='🎁';expect(buildGeometry(p).lettering.missing).toEqual(['🎁']);await expect(svgFile(p)).rejects.toThrow('неподдерживаемые');});
 it('keeps dense coverage distinctly above simple coverage',()=>{expect(Math.max(...assets.filter(a=>a.group==='simple').map(a=>a.coverage!))).toBeLessThan(15);expect(Math.min(...assets.filter(a=>a.group==='dense').map(a=>a.coverage!))).toBeGreaterThan(25);});
