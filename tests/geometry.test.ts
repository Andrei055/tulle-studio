...
import {envelopeGeometry,buildGeometry,waveDisplacement} from '../src/geometry/envelope';
...
import {assets,assetById} from '../src/library';
...
describe('Five-part envelope',()=>{
 ...
 it('protects every hinge across frame and drawing',()=>{const p=preset('peacock'),g=buildGeometry(p);for(const layer of [g.bottom,g.top])expect(area(boolean(layer,g.keepOut,'intersection'))).toBeLessThan(.005);expect(area(boolean(g.items[0].paths,g.allowed,'difference'))).toBeLessThan(.005);},30000);
...
describe('Ready-made whole drawings',()=>{
 it('exposes complete sheets instead of primitive shapes',()=>{expect(assets.filter(a=>a.group==='simple')).toHaveLength(10);expect(assets.filter(a=>a.group==='dense')).toHaveLength(10);for(const a of assets){expect(assetById(a.id).paths.flat().length).toBeGreaterThan(1000);const p=preset(a.id as 'vienna');expect(p.ornaments).toHaveLength(1);}});
 ...
 it.each(assets.map(a=>a.id))('exports a closed printable mesh: %s',kind=>{const p=preset(kind),g=buildGeometry(p);const mesh=extrude(g.top,.4);const a=meshAudit(mesh);expect(a.nonfinite).toBe(0);expect(a.degenerate).toBe(0);expect(a.badEdges).toBe(0);const expectedVolume=area(manufacturingPaths(g.top))*.4;expect(Math.abs(a.volume-expectedVolume)/expectedVolume).toBeLessThan(.0001);mesh.dispose();},30000);
...
describe('Manufacturing and persistence',()=>{
 ...
 it('retains separate PLA layers and Z settings',()=>{const p=preset('peacock');p.layers.border.visible=false;p.lettering.enabled=false;p.ornaments[0].top=false;const g=buildGeometry(p);expect(area(g.bottom)).toBeGreaterThan(0);expect(area(g.top)).toBe(0);expect(heights(p.print).resume).toBeCloseTo(.72);});
});

describe('Lettering and gentle edges',()=>{
 it('keeps Cyrillic lettering on the base and ornaments outside its clear area',()=>{const p=preset('daisy-air'),g=buildGeometry(p);expect(g.lettering.missing).toEqual([]);expect(area(g.lettering.paths)).toBeGreaterThan(20);expect(area(boolean(g.lettering.paths,g.base,'difference'))).toBeLessThan(.01);expect(area(boolean(g.items[0].paths,g.lettering.clear,'intersection'))).toBeLessThan(.01);});
 it('fits long multiline rotated text inside the base',()=>{const p=preset('blossom-air');Object.assign(p.lettering,{text:'С днём рождения!\nАнна и Михаил\n17.09.2026',x:120,y:60,rotation:31,size:32});const g=buildGeometry(p);expect(g.lettering.fitted).toBe(true);expect(g.lettering.missing).toEqual([]);expect(area(boolean(g.lettering.paths,g.base,'difference'))).toBeLessThan(.01);const mesh=extrude(g.top,.4);expect(meshAudit(mesh)).toMatchObject({badEdges:0,degenerate:0,nonfinite:0});mesh.dispose();},30000);
 it('opens earlier v2 projects without introducing an unsolicited inscription',()=>{const p=preset('peacock');const old=JSON.parse(JSON.stringify(p));delete old.lettering;expect(projectFromJSON(JSON.stringify(old)).lettering.enabled).toBe(false);});
 it('protects export against unsupported lettering',async()=>{const p=preset('orchid-air');p.lettering.text='🎁';expect(buildGeometry(p).lettering.missing).toEqual(['🎁']);await expect(svgFile(p)).rejects.toThrow('неподдерживаемые');});
 it('keeps dense botanical motifs distinct from simple ones without a grid construction',()=>{expect(Math.max(...assets.filter(a=>a.group==='simple').map(a=>a.coverage!))).toBeLessThan(20);expect(Math.min(...assets.filter(a=>a.group==='dense').map(a=>a.coverage!))).toBeGreaterThan(18);expect(assets.every(a=>a.construction==='motifs')).toBe(true);});
 it('has smooth wave endpoints even with a shifted phase',()=>{const e={...base().envelope,phase:90,amplitude:6};expect(waveDisplacement(0,120,e)).toBe(0);expect(waveDisplacement(1,120,e)).toBe(0);expect(waveDisplacement(.0001,120,e)/.012).toBeLessThan(.002);expect(waveDisplacement(.9999,120,e)/.012).toBeLessThan(.002);});
});
